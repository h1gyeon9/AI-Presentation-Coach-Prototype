...
function buildSystemInstruction(mode, profile = {}, payload = {}) {
  ...
  const personaStyleMap = {
    hr: "채용 담당자로서 지원 동기, 팀 문화 적합도, 의사소통 스타일을 친근하지만 꼼꼼하게 검증합니다. '왜 이 회사인가', 가치관, 협업 경험을 중심으로 질문합니다.",
    field: "현업 팀장으로서 구체적 업무 역량과 실무 문제 해결 방식을 검증합니다. '실제로 어떻게 했는지', 수치와 결과를 중심으로 꼬리질문합니다.",
    pressure: "의도적으로 도전적인 질문과 반론으로 압박하며 위기 대처 능력을 검증합니다. 답변의 허점을 지적하고 강도 높은 꼬리질문을 이어갑니다.",
    executive: "임원으로서 성장 가능성, 리더십, 비전과 가치관을 거시적 관점에서 검증합니다. 개인 경험보다 판단 기준, 조직 기여 방향, 장기 목표를 중심으로 질문합니다.",
  };
  const personaStyle = personaStyleMap[profile.persona] || "";
  return [
